-- =====================================================================
-- RUSH Car Wash — los rechazos de carros de prueba no deben contar
--
-- Encontrado el 20/jul/2026 al borrar un carro de prueba y notar que su
-- rechazo salia en el reporte, a nombre de una persona.
--
-- El reporte tenia dos filtros distintos para la misma pregunta "que
-- cuenta como del dia":
--
--   del_dia      -> not es_prueba  and cancelado_en is null   (bien)
--   rechazos_dia -> solo por fecha                            (mal)
--
-- Efecto: un carro de prueba NO contaba como vehiculo lavado, pero sus
-- rechazos SI se le anotaban a la persona que aparecia secandolo. O sea
-- que probar la app le ensuciaba el historial a alguien real.
--
-- Lo mismo con los cancelados: una devolucion cancela el carro y sale de
-- todas las cuentas, pero su rechazo se quedaba.
--
-- Ahora rechazos_dia se une a carros y aplica los MISMOS dos filtros.
--
-- Leccion, que es la parte que vale: cuando dos consultas responden la
-- misma pregunta ("que es del dia") con condiciones distintas, tarde o
-- temprano se desfasan. Es el mismo error que el CLAUDE.md ya documenta
-- en la seccion 12.1 sobre express y aspirado, donde la solucion fue
-- tener UNA sola funcion. Aqui todavia son dos listas de condiciones
-- copiadas; si esto se vuelve a tocar, conviene sacarlas a un solo lugar.
--
-- Lo demas es identico a la 037.
-- =====================================================================
