-- =====================================================================
-- RUSH Car Wash — los rechazos entran al reporte diario
--
-- Es la razon de ser de la funcion de rechazo: "para que en los reportes
-- veamos que secador tiene rechazos constantes y necesita mas
-- entrenamiento".
--
-- Se cuenta POR PERSONA, no por equipo, a proposito: un equipo de dos no
-- dice cual de los dos fallo, y a quien hay que entrenar es a una persona.
--
-- Nota: con un dia de datos esto va a salir en cero. Es normal — la
-- funcion existe desde antes que el dato.
-- =====================================================================
...
begin
  ...
  -- Los rechazos se cuentan por CUANDO SE RECHAZO, no por cuando entro el
  -- carro. Un carro de ayer que se rechaza hoy es un rechazo de hoy: es
  -- el dia en que alguien tuvo que rehacer el trabajo.
  rechazos_dia as (
    select r.*
  ...
  -- count(distinct grupo) = eventos de rechazo. count(*) contaria un
  -- rechazo de dos personas como dos.
  ...
  -- Por PERSONA. Se agrupa por empleado_id cuando existe; si no (captura
  -- manual o fila vieja), por el nombre congelado.
  por_secador as (
  ...
  select jsonb_build_object(
    ...
    -- Eventos de rechazo del dia, y cuantos carros distintos tuvieron al
    -- menos uno. Los dos numeros importan: 8 rechazos en 8 carros no es
    -- lo mismo que 8 rechazos en 2 carros.
