-- =====================================================================
-- 065 — Contexto: cuantos carros le entraron ENCIMADOS a cada equipo
--
-- El punto 8 (cola virtual): cuando a una persona le entra un carro y ya
-- estaba secando otro, el segundo TIENE que esperar a que se desocupe, pero
-- el reloj cuenta desde que se lo asignaste. Ese rato de espera se cuenta
-- como secado e infla el tiempo del que MAS carga — el reporte lo hace ver
-- lento cuando el problema es que le echaste mas carros de los que puede a
-- la vez.
--
-- El dueño (24/jul) descarto la solucion que exige un toque mas del
-- supervisor (son de la tercera edad y pierden la huella del carro tras
-- asignarlo). Escogio la version B: NO tocar ningun promedio, solo MOSTRAR
-- el contexto para que el numero no se lea mal.
--
-- Asi que aqui NO se cambia ningun tiempo. Solo se agrega, por equipo y en
-- general, cuantos carros "arrancaron encimados": al momento de asignarlos,
-- alguno de sus secadores ya traia otro carro sin entregar. Se saca SOLO de
-- los datos que ya existen (hora de asignacion vs. entrega del otro carro),
-- cero accion del supervisor.
--
-- Es un CONTEO (si/no estaba ocupado), no una resta — por eso no se va a
-- negativo con el caso de secado en paralelo (carro 109) que hizo pausar la
-- version que corregia el numero. Un carro en paralelo simplemente sale
-- marcado como "encimado", que para contexto es cierto: la persona si traia
-- otro a la vez.
--
-- Base: la 064. Los promedios de secado quedan IDENTICOS (verificado). Solo
-- se agregan campos nuevos: 'encimados' por equipo y en general.
-- =====================================================================
...
comment on function public.reporte_del_rango(date, date) is
  ...
  '< 3 min fuera del promedio (064). Muestra cuantos carros arrancaron '
  'encimados, por equipo y en general, SIN tocar ningun promedio (065).';
