#!/usr/bin/env bash
# Arma el arnes del front (supervisor Y caja) en una carpeta temporal.
#
# 🔑 NO copia el HTML a mano: lo EXTRAE de docs/ en cada corrida,
...
awk 'NR==4{print; print "<script src=\"stub.js\"></script>"; next} {print}' \
    "$raiz/docs/caja.html" > "$destino/caja.html"
...
for pantalla in index caja; do
  if ! diff <(sed '5d' "$destino/$pantalla.html") "$raiz/docs/$pantalla.html" > /dev/null; then
    echo "FALLO: el arnes no coincide con docs/$pantalla.html" >&2
    exit 1
  fi
done
...
echo "Y abre http://localhost:8777/  (o /caja.html)"
